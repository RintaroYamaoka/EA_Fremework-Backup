//+------------------------------------------------------------------+
//|                                 TestMA LimitedTest FixedSLTP.mq5 |
//|                                                   RintaroYamaoka |
...
input ulong MagicNo = 2025110601;
    ...
    void OnTickMethod() override
    {
        ...
        if (cross_up)
        {
            ...
            if(!has_long && session.IsActiveDay())
                order.Entry(true, LotSize, Fixed_SLTP, Fixed_SLTP);
        ...
        if (cross_down)
        {
            ...
            if(!has_short && session.IsActiveDay())
                order.Entry(false, LotSize, Fixed_SLTP, Fixed_SLTP);
